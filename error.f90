...
!-----------------------------------------------------------------------------!
! >>> Differentiation schemes + Error analysis
...
implicit none
integer::j,n,k,kmax
real*8 ::h,e1,e2,e3,e4
real*8,allocatable::x(:),u(:),upe(:)
real*8,allocatable::up1(:),up2(:),up3(:),up4(:)

! Writing data to text file
open(11, file="error.plt")
write(11,*)'variables ="h","f1","c2","c4","pade4"'
...
h = 4.0d0/dfloat(n)
    ...
    !compute error at the midpoint x=4 (i=n/2)

    e1 = dabs(up1(n/2) - upe(n/2))
    e2 = dabs(up2(n/2) - upe(n/2))
    e3 = dabs(up3(n/2) - upe(n/2))
    e4 = dabs(up4(n/2) - upe(n/2))
     
	write(11,*) h,e1,e2,e3,e4
    

	deallocate(x,u,upe)
    deallocate(up1,up2,up3,up4)
